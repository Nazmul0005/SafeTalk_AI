...
class TranscriptionWithModerationRequest(BaseModel):
    """Schema for transcription with moderation request"""
...
class TranscriptionWithModerationResponse(BaseModel):
    """Schema for transcription with moderation response"""
    ...
    # Moderation results
    moderation_flagged: bool = Field(..., description="Whether the content was flagged by moderation")
    moderation_reason: Optional[str] = Field(None, description="Reason for moderation flag if any")
    is_safe: bool = Field(..., description="Whether the content is safe to use")


class TranscriptionWithModerationError(BaseModel):
    """Schema for transcription with moderation error response"""
    ...
    stage: str = Field(..., description="Stage where error occurred (transcription/moderation)")